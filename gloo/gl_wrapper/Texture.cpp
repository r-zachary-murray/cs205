...
void Texture::Initialize(const TextureConfig& config) {
  GL_CHECK(glGenTextures(1, &handle_));//Generate a new texture handle

  BindToUnit(0);//Bind the new handle to make it active glBindTexture(GL_TEXTURE_2D, handle).
  ...
  //Configure the texture minification filter using glTexParameteri(GL TEXTURE 2D, GL TEXTURE MIN FILTER, GL LINEAR).
  // This enables basic bilinear filtering.
