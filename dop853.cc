 ...
 * \param[in] dof_ the number of degrees of freedom in the problem. */
dop853::dop853(int dof_) : dof(dof_), nmax(16777216), nstiff(4), fac1(1/3.0),
    fac2(6.0), safe(0.9), beta(0.0), uround(2.3e-16), atoli(1e-14),
    rtoli(0), w(new double[dof]), ww1(new double[dof]), k1(new double[dof]),
    k2(new double[dof]), k3(new double[dof]), k4(new double[dof]),
    k5(new double[dof]), k6(new double[dof]), k7(new double[dof]),
    k8(new double[dof]), k9(new double[dof]), k10(new double[dof]),
    rc1(new double[dof]), rc2(new double[dof]), rc3(new double[dof]),
    rc4(new double[dof]), rc5(new double[dof]), rc6(new double[dof]),
    rc7(new double[dof]), rc8(new double[dof]) {}
...
int dop853::solve(double tstart,double tend,int snaps) {
    ...
    while(true) {
        ...
        if(err<=1.0) {
            ...
            while(csn<nsn) {csn++;dense(ww1,tstart+csn*sf);output(csn,ww1);}
...
void dop853::step12(double *p1) {
    ...
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*a21*k1[i];
    ff(t+c2*h,ww1,k2);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a31*k1[i]+a32*k2[i]);
    ff(t+c3*h,ww1,k3);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a41*k1[i]+a43*k3[i]);
    ff(t+c4*h,ww1,k4);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a51*k1[i]+a53*k3[i]+a54*k4[i]);
    ff(t+c5*h,ww1,k5);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a61*k1[i]+a64*k4[i]+a65*k5[i]);
    ff(t+c6*h,ww1,k6);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a71*k1[i]+a74*k4[i]+a75*k5[i]+a76*k6[i]);
    ff(t+c7*h,ww1,k7);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a81*k1[i]+a84*k4[i]+a85*k5[i]+a86*k6[i]+a87*k7[i]);
    ff(t+c8*h,ww1,k8);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a91*k1[i]+a94*k4[i]+a95*k5[i]+a96*k6[i]+a97*k7[i]+a98*k8[i]);
    ff(t+c9*h,ww1,k9);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a101*k1[i]+a104*k4[i]+a105*k5[i]
                   +a106*k6[i]+a107*k7[i]+a108*k8[i]+a109*k9[i]);
    ff(t+c10*h,ww1,k10);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a111*k1[i]+a114*k4[i]+a115*k5[i]+a116*k6[i]
                   +a117*k7[i]+a118*k8[i]+a119*k9[i]+a1110*k10[i]);
    ff(t+c11*h,ww1,k2);
    tph=t+h;
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a121*k1[i]+a124*k4[i]+a125*k5[i]+a126*k6[i]+a127*k7[i]
...
double dop853::error_estimation() {
    ...
    deno=err+0.01*err2;
    return err*sqrt(1.0/(deno<=0.0?dof:deno*dof));
}
...
void dop853::dense_output() {
    ...
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a141*k1[i]+a147*k7[i]+a148*k8[i]+a149*k9[i]
                   +a1410*k10[i]+a1411*k2[i]+a1412*k3[i]+a1413*k4[i]);
    ff(t+c14*h,ww1,k10);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a151*k1[i]+a156*k6[i]+a157*k7[i]+a158*k8[i]
                   +a1511*k2[i]+a1512*k3[i]+a1513*k4[i]+a1514*k10[i]);
    ff(t+c15*h,ww1,k2);
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*(a161*k1[i]+a166*k6[i]+a167*k7[i]+a168*k8[i]
...
void dop853::dense(double *ws,double ti) {
    ...
    for(int i=0;i<dof;i++) ws[i]=rc1[i]+s*(rc2[i]+s1*(rc3[i]+s*(rc4[i]
...
double dop853::hinit(double hmax,double posneg) {
    ...
    for(i=0;i<dof;i++) ww1[i]=w[i]+h*k1[i];
    ff(t+h,ww1,k2);nff++;
